...
eventOid = "SE_DSDROPOUT"
for event in selectedStudySubject.events:
    if event.eventDefinitionOID == eventOid:
        selectedStudyEvent = event

        crfOid = "F_DSDRO_ENV10"
        for crf in event.forms:
            if crf.oid == crfOid:
                selectedCrf = crf

                itemOid = "I_DSDRO_DODETAILS"
                for item in crf.items:
                    if item.oid == itemOid:
                        selectedItem = item


item.value = "Very good reason for Drop-out"

svcOdm = OdmFileDataService()
odm = svcOdm.generateOdmXmlForStudyItem(
    selectedStudy.oid,
...
importResult = ocWebServices.importODM(odm)
